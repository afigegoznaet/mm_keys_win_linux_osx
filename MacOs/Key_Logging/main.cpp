...
int main(int argc, const char *argv[]) {
    ...
    CGEventMask eventMask = NX_SYSDEFINEDMASK;
    CFMachPortRef eventTap = CGEventTapCreate(
        kCGHIDEventTap, kCGHeadInsertEventTap, (CGEventTapOptions) 0, eventMask, CGEventCallback, nullptr
    );
...
CGEventRef CGEventCallback(CGEventTapProxy proxy, CGEventType type, CGEventRef event, void *refcon) {
    //printf("Got key type\n");
    if (type != kCGEventKeyDown && type != kCGEventFlagsChanged && type != kCGEventKeyUp && type >9 &&type !=NX_SYSDEFINED) {
        printf("Unkown type\n");
        printf("%d\n",type);
        return event;
    }
    ...
    keyCode = (CGKeyCode) CGEventGetIntegerValueField(event, (CGEventField)150);
    if(65535 != keyCode)
        return event;
    ...
    for(int i=0;i<65535;i++){
        keyCode = (CGKeyCode) CGEventGetIntegerValueField(event, (CGEventField)i);
        if(0==keyCode)
            continue;
        printf("%d - %d\n",i, keyCode);
